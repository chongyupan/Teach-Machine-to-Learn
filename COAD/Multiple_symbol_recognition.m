...
shot_num=1;
TopN=5;            % TopN个最有可能的选项: Multiple-symbol: 5;
primitives_num=5;     % 笔画基元种类数: COAD:5  
...
for echo=1:10     %1:10
    str=['../Datasets/COAD/',num2str(way_num),'way',num2str(shot_num),'shot_epho',num2str(echo),'_dataset.mat'];
        load(str);
    str=['../Datasets/COAD/object_',num2str(way_num),'way',num2str(shot_num)','shot_epho',num2str(echo),'.mat'];
        load(str);
    Top1_True_Count=0;
    TopN_True_Count=0;
    ...
    for sample_no=(1:1100)+00                  % 1:1100共11段: (1:100)+500
        fprintf('Processing %dth/10 echo, %dth/100 sample;\n',echo,sample_no);
...
str_save=['../Datasets/COAD/Acc_for_Multiple_symbol_recognition_',num2str(way_num),'way',num2str(shot_num),'shot.mat'];
